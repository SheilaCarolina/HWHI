	...
	try{
		...
		if($arquivo != "none"){
			if(move_uploaded_file($arquivo["tmp_name"],"../Resources/Arquivos/".$arquivo["name"])){
				...
				$sql = $sql = "INSERT INTO `tema1_o_que_e_a_vida`(`idUser`, `titulo`, `nomeAutor`, `resumo`, `file`, `area`, `subarea`, `lattes`, `areaAutor`, `subAreaAutor`) 
				VALUES ('".$_SESSION['idUser']."','$titulo','$nome', '$resumo', '$arquivo', '$area', '$subarea', '".$lattes."', '".$areaAutor."', '".$subareaAutor."')";
				$conn -> exec($sql); 
